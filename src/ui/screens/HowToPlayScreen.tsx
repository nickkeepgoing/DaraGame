...
export function HowToPlayScreen({ busy, error, nickname, onStart, onLeaderboard }: Props) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-y-auto p-4">
      <div className="animate-pop-in panel w-full max-w-2xl rounded-3xl p-7">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h2 className="font-display text-2xl font-bold text-dusk-100">
              พร้อมแล้วนะ {nickname} 🦕
            </h2>
            <p className="text-sm text-dusk-200/75">
        ...
        <div className="mb-5 space-y-2">
          {CONTROLS.map((c) => (
            <div
              key={c.keys}
              className="flex flex-wrap items-center gap-3 rounded-xl bg-white/5 px-4 py-2.5"
            >
              <kbd className="tabular shrink-0 rounded-lg bg-night-900 px-3 py-1 font-mono text-sm text-dusk-200">
                {c.keys}
              </kbd>
        ...
        {error && (
          <p className="mb-4 rounded-xl border border-lava-500/40 bg-lava-600/15 px-4 py-2.5 text-sm text-dusk-100">
            {error}
          </p>
        )}
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <button
            onClick={onStart}
            disabled={busy}
            className="btn-primary flex-1 rounded-xl py-3.5 text-lg"
          >
